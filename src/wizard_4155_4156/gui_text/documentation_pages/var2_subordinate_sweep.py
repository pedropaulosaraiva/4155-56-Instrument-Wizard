...
| **Number of points** | How many secondary points to measure, chosen with the spinner. Allowed range: **1 to 128**. |
...
- **No Stop field.** There is no stop value to enter — the final VAR2 value is
  computed from **Start + Step × (Number of points − 1)** (the first point is the
  Start value). The card checks that this derived endpoint still falls within the
  assigned channel's valid range and flags it if it does not.
...
- **Counts toward the total.** The number of VAR2 points multiplies the VAR1 point
